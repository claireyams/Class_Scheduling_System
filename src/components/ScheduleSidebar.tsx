...
export function ScheduleSidebar() {
  const { entries, totalUnits, removeCourse, clearSchedule, hasConflict } = useSchedule();
          ...
          >
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                {conflicted && (
                  <span className="rounded bg-red-600 px-1.5 py-0.5 text-[10px] font-bold uppercase leading-tight text-white">
                    Clash
                  </span>
                )}
              </div>
              ...
              <p className="mt-0.5 truncate text-xs text-muted">
                {entry.section.room} · {entry.section.instructor}
              </p>
            </div>
            <button
              type="button"
